  ...
  <main>
    <PaperDoc>
      <div class="grid grid-cols-3 justify-center">
        ...
        <div class="flex items-start">
          <div class="border-2 border-black p-2">
            <p>
              รับที่ <FiLL :data="data?.receivedLocation" /></p>
            <p>วันที่ <FiLL :data="data?.dateReceived" /></p>
          </div>
        </div>
        ...
        <div class="flex flex-col items-end">
          <div class="border-2 border-black p-2">
            <p>หนังสือกู้ที่  <FiLL :data=" data?.receivedLocation "/></p>
            <p>วันที่ <FiLL :data="data?.dateReceived" /></p>
            <p>บัญชีเงินกู้ที่  <FiLL :data=" data?.number "/></p>
          </div>
        </div>
      ...
      <div class="flex flex-col ml-10">
        <p>ข้าพเจ้า <FiLL :data=" data?.fname  "/> <FiLL :data=" data?.lname "/> สมาชิกเลขทะเบียนที่ <FiLL :data=" data?.idmember  "/></p>
        <p>รับราชการหรือทำงานประจำในตำแหน่ง <FiLL :data=" data?.position  "/> สังกัด <FiLL :data=" data?.affili  "/></p>
        <p>ได้รับเงินได้รายเดือน <FiLL :data=" data?.salary "/> บาท ขอเสนอคำขอกู้เงินเพื่อเหตุฉุกเฉินดังต่อไปนี้</p>
      </div>

      <!-- Right-aligned text -->
      <div class="flex flex-col ml-10">
        <p>ข้อ ๑. ข้าพเจ้าขอกู้เงินของสหกรณ์ จำนวน <FiLL :data=" data?.loan "/> บาท (<MoneyText :data=" data?.full  "/>) โดยจะนำไปใช้เพื่อการดังนี้ <FiLL :data=" data?.use  "/></p>
      </div>

      <!-- Right-aligned text -->
      <div class="flex flex-col ml-10">
        <p>ข้อ ๒. ถ้าข้าพเจ้าได้รับเงินกู้ ข้าพเจ้าขอส่งเงินกู้คืน ดังนี้</p>
      </div>

      <div class="flex flex-col ml-10">
        <p class="text-end">(๑)
          ภายในวันสิ้นเดือน <FiLL :data="data?.dateReceived" /> ต้นเงิน <FiLL :data="data?.moneytree"/> บาท
          พร้อมด้วยดอกเบี้ย
        </p>
        <p class="text-end">(๒)
          ภายในวันสิ้นเดือน <FiLL :data="data?.dateReceived" /> ต้นเงิน <FiLL :data="data?.moneytree"/> บาท
...
const { data } = await useFetch<any>('/api/MintSampleData');
